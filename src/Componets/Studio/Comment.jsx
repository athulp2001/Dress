import React from 'react'
import './Studio.css'
const Comment = () => {
  return (
    <div style={{backgroundColor:'white',}}>
      <div style={{display:'flex', justifyContent:'space-between', width:'94%', marginLeft:'3%', paddingTop:'5px',paddingBottom:'5px'}}>
<div>
<svg width="24" height="24" viewBox="0 0 24 24" fill="rgba(40, 44, 63, 1)"><g fill="none" fill-rule="evenodd"><g fill="rgba(40, 44, 63, 1)" fill-rule="nonzero"><g><path d="M16.458 2.75c3.193 0 5.792 2.557 5.792 5.702 0 .665-.108 1.361-.32 2.076l-.081.254.05.095c.164.33.273.684.322 1.05l.022.222.007.224c0 1.575-.934 3.133-2.295 4.519-1.215 1.238-2.639 2.162-3.565 2.162-.288 0-.612-.087-.966-.252l-.044-.021-.123.087c-1.057.725-2.048 1.226-2.873 1.352l-.202.023-.182.007c-.746 0-1.716-.412-2.9-1.216-1.065-.723-2.224-1.721-3.265-2.813-2.351-2.465-4.085-5.227-4.085-7.769 0-3.145 2.6-5.702 5.792-5.702 1.106 0 2.182.308 3.11.89.344.216.662.466.95.745l.211.215.187.211.187-.211c.205-.22.427-.425.665-.61l.243-.181.253-.168c.928-.583 2.004-.891 3.11-.891zm0 1.653c-1.591 0-3.01.873-3.707 2.279-.14.283-.432.463-.751.463s-.61-.18-.75-.463c-.698-1.406-2.117-2.28-3.708-2.28-2.272 0-4.12 1.818-4.12 4.05 0 1.757 1.354 4.249 3.63 6.635 1.914 2.008 4.133 3.51 4.948 3.51.327 0 .985-.26 1.792-.755l.094-.06c-.28-.233-.556-.483-.815-.74l-.254-.259c-1.317-1.38-2.287-2.937-2.287-4.41 0-1.892 1.563-3.429 3.481-3.429.834 0 1.622.287 2.242.801l.137.122.138-.122c.563-.467 1.265-.747 2.015-.793l.226-.008c.575 0 1.129.14 1.624.398l.093.052.033-.186c.026-.173.044-.343.053-.51l.006-.246c0-2.232-1.848-4.05-4.12-4.05zm2.311 6.194c-.7 0-1.322.383-1.628 1-.14.284-.432.463-.75.463-.32 0-.61-.179-.751-.462-.306-.618-.93-1-1.629-1-.998 0-1.809.797-1.809 1.775 0 .834.682 2.071 1.831 3.276.985 1.033 2.07 1.752 2.357 1.752.314 0 1.404-.68 2.366-1.66 1.162-1.184 1.822-2.42 1.822-3.368 0-.978-.81-1.776-1.809-1.776z" transform="translate(-12 -622) translate(12 622)"></path></g></g></g></svg>
<svg width="24" height="24" viewBox="0 0 24 24" fill="#282C3F"><g fill="none" fill-rule="evenodd"><g fill="#282C3F" fill-rule="nonzero" stroke="rgba(40, 44, 63, 0.9)" stroke-width="0.5"><g><g><g><path d="M14.656 12.329c-1.104 0-2.088.526-2.718 1.339l-5.272-2.945c.124-.355.192-.737.192-1.132 0-.398-.068-.777-.196-1.135l5.268-2.941c.626.817 1.614 1.347 2.722 1.347 1.889 0 3.43-1.538 3.43-3.431S16.546 0 14.653 0c-1.893 0-3.431 1.538-3.431 3.43 0 .4.068.782.195 1.136L6.152 7.507C5.527 6.687 4.54 6.16 3.431 6.16 1.542 6.16 0 7.698 0 9.591s1.542 3.431 3.435 3.431c1.108 0 2.096-.53 2.725-1.35l5.268 2.944c-.127.359-.2.745-.2 1.144 0 1.888 1.539 3.43 3.432 3.43s3.43-1.538 3.43-3.43c0-1.893-1.541-3.431-3.434-3.431zm0-11.25c1.299 0 2.355 1.057 2.355 2.356S15.955 5.79 14.656 5.79c-1.3 0-2.355-1.056-2.355-2.355 0-1.3 1.06-2.355 2.355-2.355zM3.435 11.947c-1.3 0-2.355-1.056-2.355-2.355s1.056-2.355 2.355-2.355S5.79 8.292 5.79 9.591c0 1.3-1.06 2.355-2.355 2.355zm11.22 6.165c-1.298 0-2.354-1.056-2.354-2.355 0-1.3 1.056-2.355 2.355-2.355s2.355 1.056 2.355 2.355-1.056 2.355-2.355 2.355z" transform="translate(-89 -664) translate(12 664) translate(77) translate(3 2)"></path></g></g></g></g></g></svg>
</div>
<div>
<svg width="24" height="24" viewBox="0 0 24 24" fill="rgba(40, 44, 63, 1)"><g fill="none" fill-rule="evenodd"><g fill="rgba(40, 44, 63, 1)" fill-rule="nonzero"><g><g><g><path d="M15 0H3.002C1.344 0 0 1.344 0 3.001V17.25c0 .644.759.988 1.244.564l5.506-4.818 5.506 4.818c.485.424 1.244.08 1.244-.564v-8.25h3.75c.414 0 .75-.335.75-.75V3C18 1.344 16.657 0 15 0zm-3 15.596l-4.756-4.161c-.283-.248-.705-.248-.988 0L1.5 15.595V3.002c0-.829.672-1.501 1.501-1.501h9.4l-.031.057-.01.018-.059.113-.013.027-.047.104-.007.017c-.017.04-.032.08-.047.12l-.01.03c-.014.036-.027.073-.038.11l-.005.015c-.013.042-.025.083-.036.125l-.008.031-.027.12-.003.01-.023.13-.005.033-.016.128v.006c-.005.045-.008.09-.01.135l-.003.033L12 3v12.596zm4.5-8.097h-3V3c0-.828.671-1.5 1.5-1.5s1.5.672 1.5 1.5v4.5z" transform="translate(-324 -622) translate(12 622) translate(312) translate(3 3)"></path></g></g></g></g></g></svg>
</div>

      </div>


      <div style={{textAlign:'left', width:'94%', marginLeft:'3%', paddingTop:'5px',paddingBottom:'5px'}}>
    <h6 className='hashTagH6'><span className='followComentSpan'>Myntra Finde Resort Wear Maxi Dress</span > #Revies #womencasualware #dresses # floralprint #maxidress #street9 <span className='seeMoreSpan'>...See More</span></h6>
    </div>


    </div>
  )
}

export default Comment
